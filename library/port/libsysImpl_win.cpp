 ...
 */
int __syswin_getLocaleEntryIndex(LANGID langID)
{
	int index = -1;
	int tries = 0;
	do 
...
string __syswin_getEncodingInternal(int index)
{
	string __ret_encoding;
  ...
  if (strcmp(ret, "MS950") == 0) 
  {
		...
		{
			__ret_encoding.assign("MS950_HKSCS");
			return __ret_encoding;
		}
  } 
  else 
  {
    ...
    {
      char systemPath[OPENCODE_PATH_MAX + 1];
      char* gb18030Font = "\\FONTS\\SimSun18030.ttc";
      FILE *f = NULL;
      if (GetWindowsDirectory(systemPath, OPENCODE_PATH_MAX + 1) != 0 && 
	        strlen(systemPath) + strlen(gb18030Font) < OPENCODE_PATH_MAX + 1) 
	    {
	       strcat(systemPath, "\\FONTS\\SimSun18030.ttc");
	       if ((f = fopen(systemPath, "r")) != NULL) 
	       {
	         fclose(f);
					 __ret_encoding.assign("GB18030");
					 return __ret_encoding;
	       }
      }
    }  
 ...
 */
bool __syswin_getHomeFromRegistry(char* pathname,DWORD& pathsize)
{
  HKEY key;
  int rc;
  DWORD type;
  ...
  pathname[0] = 0;
  rc = ::RegQueryValueEx(key, "Desktop", 0, &type, (LPBYTE)pathname, &pathsize);
 ...
 void SystemDoImpl::initializeLocale()
{
	...
	{
    int index = __syswin_getLocaleEntryIndex(langID);

    /*
     * if we didn't find the LCID that the system returned to us,
     * we don't have a  locale ID that corresponds to it. 
     * Fall back on en_US.
     */
    if (index == -1) 
    {
      LOGGING_DEBUG("SystemDoImpl::<init>|set ${file.encoding} to {Cp1252}");  
      __props.setProperty("file.encoding","Cp1252");

      LOGGING_DEBUG("SystemDoImpl::<init>|set ${user.language} to {en}");
      __props.setProperty("user.language","en");  

      LOGGING_DEBUG("SystemDoImpl::<init>|set ${user.country} to {US}");
      __props.setProperty("user.country","US");
    } 
    else 
    {	
      string __file_encoding = __syswin_getEncodingInternal(index);
      LOGGING_DEBUG("SystemDoImpl::<init>|set ${file.encoding} to {" << __file_encoding << "}");  
      __props.setProperty("file.encoding",__file_encoding);

			/* otherwise, look up the corresponding locale ID from
			 * the list of  locale IDs and set up the system properties
			 * accordingly.
			 */
			const char* __lang_tmp = langIDMap[index].localeID;
			size_t __lang_tmp_size = strlen(__lang_tmp);
			
			char* lang = new char[__lang_tmp_size + 1];
			strncpy(lang,__lang_tmp,__lang_tmp_size);
			lang[__lang_tmp_size] = '\0';
      ...
      LOGGING_DEBUG("SystemDoImpl::<init>|set ${user.language} to {" << lang << "}");
      __props.setProperty("user.language",lang);  
    
      LOGGING_DEBUG("SystemDoImpl::<init>|set ${user.country} to {" << ctry << "}");
      __props.setProperty("user.country",ctry);
      
      if(strlen(variant) > 0)
      {
        LOGGING_DEBUG("SystemDoImpl::<init>|set ${user.variant} to {"<< variant <<"}");
        __props.setProperty("user.variant",variant);
      }
      delete []lang;
    }
	}
...
void SystemDoImpl::initializeOthers()
{
  /*hardware properties */
  {
    ...
    processors_conf = sysInfo.dwNumberOfProcessors;
    processors_onln = sysInfo.dwNumberOfProcessors;
  ...
  {  
    ...
    switch (ver.dwPlatformId) 
    {
      ...
      {
        __props.setProperty("os.name","Windows 3.1");
      }
      break;
      case VER_PLATFORM_WIN32_WINDOWS:
      {
        if (ver.dwMajorVersion == 4) 
        {
          switch (ver.dwMinorVersion) 
          {
            case 0:
            {
              __props.setProperty("os.name","Windows 95");
            }
            break;
            case 10:
            {
              __props.setProperty("os.name","Windows 98");
            }
            break;
            case 90:
            {
              __props.setProperty("os.name","Windows Me");
            }
            break;
            default:
            {
              __props.setProperty("os.name","Windows 9X (unknown)");
            }
            break;  
          }
        } 
      ...
      case VER_PLATFORM_WIN32_NT:
      {
        if (ver.dwMajorVersion <= 4) 
        {
          __props.setProperty("os.name","Windows NT");
        } 
        else if (ver.dwMajorVersion == 5) 
        {
          switch (ver.dwMinorVersion) 
          {
            case 0:
            {
              __props.setProperty("os.name","Windows 2000");
            }
            break;
            case 1:
            {
              __props.setProperty("os.name","Windows XP");
            }
            break;
            case 2:
            {
              __props.setProperty("os.name","Windows 2003");
            }
            break;
            default:
            {
              __props.setProperty("os.name","Windows NT (unknown)");
            }
            break;
          }
        } 
      ...
      default:
      {
        __props.setProperty("os.name","Windows (unknown)");
      }
      break;
    }
    ...
    __props.setProperty("os.version",buf);
 
    char tmpdir[OPENCODE_PATH_MAX + 1] = {0};
    /* we might want to check that this succeed */
    ::GetTempPath(OPENCODE_PATH_MAX + 1, tmpdir);
    __props.setProperty("os.tmpdir",tmpdir);
  }

  /* file system properties */
  {
    __props.setProperty("file.separator",FILE_SEPARATOR);
    __props.setProperty("path.separator",FILE_PATHSEPARATOR);
    __props.setProperty("line.separator",FILE_LINESEPARATOR);
  }  
  ...
  /* user properties */
  {
    ...
    __props.setProperty("user.name",buf);

    
    char path[OPENCODE_PATH_MAX  + 1] ={0};
    DWORD size = OPENCODE_PATH_MAX  + 1;
    bool __userhome_found = __syswin_getHomeFromRegistry(path,size);
    if(__userhome_found)
    {
      __props.setProperty("user.home",path);
    }
    else
    {
      __props.setProperty("user.home","C:\\");
    } 
  }
...
void SystemDoImpl::parseCommandLine(string& appcmd,string& appenv,const string& cmdline)
{
	...
	if(cmdline[0] == '\"')
	{
		const char* __sbuf = (const char*)cmdline.data();
		size_t __sbuf_size = cmdline.size();
		
		bool __isInstring = true;
		char __sb_char = 0;
		size_t i = 1;
		...
		for(; i < __sbuf_size; ++i)
		{
		  ...
		  if(__isInstring)
		  {
		  	...
		  	{
		  		__isInstring = false;
		  		break; //find application command
		    }	
		    else
		    {
		    	__sb << __sb_char;
		...
		}//~end for(; i < __sbuf_size; ++i)
    
    appcmd = __sb.str();
    
    if(cmdline.size() > (appcmd.size() + 2))
    {
      appenv = 	cmdline.substr(appcmd.size() + 2);
    }	
