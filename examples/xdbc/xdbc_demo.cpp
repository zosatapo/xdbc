  ...
  for(int i = 1;i <= __sp_meta->getColumnCount(__XDBC_HOOK_INVOKE_ONCE); ++i)
  {
    ...
    switch(__col_type)
    {
      ...
      case XDBC_TYPE_LONGVARCHAR:
      {
        ...
        {
          char* __p_data_char = new char[__sizeBytes+1];
          __p_data_char[__sizeBytes]='\0';
          memcpy(__p_data_char,__p_char,__sizeBytes);
          cout << __column_name << "  : <" << __sizeBytes << "> " << __p_data_char << endl;
          cout << MemoryKit::numDump(__p_char,__sizeBytes) << endl;
          delete []__p_data_char;
        }  
      }
      break;  
